...
export default function DiscussionsModerationClient({
  ...
  const [filterStatus, setFilterStatus] = useState<"all" | "flagged" | "removed" | "active">(() => {
    if (typeof window !== "undefined") {
      return (window.localStorage.getItem("discussions_filterStatus") as any) || "all";
    }
    return "all";
  });
  const [sortBy, setSortBy] = useState<"recent" | "flagged">(() => {
    if (typeof window !== "undefined") {
      return (window.localStorage.getItem("discussions_sortBy") as any) || "recent";
    }
    return "recent";
  });
  ...
  const [itemsPerPage, setItemsPerPage] = useState<10 | 25 | 50 | 100>(() => {
    if (typeof window !== "undefined") {
      const saved = window.localStorage.getItem("discussions_itemsPerPage");
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      window.localStorage.setItem("discussions_itemsPerPage", String(itemsPerPage));
    }
  }, [itemsPerPage]);
  useEffect(() => {
    if (typeof window !== "undefined") {
      window.localStorage.setItem("discussions_filterStatus", filterStatus);
    }
  }, [filterStatus]);
  useEffect(() => {
    if (typeof window !== "undefined") {
      window.localStorage.setItem("discussions_sortBy", sortBy);
    }
  }, [sortBy]);
  ...
  const filteredDiscussions = useMemo(() => {
    ...
    if (debouncedSearch) {
      filtered = filtered.filter(d => 
        ...
        d.clubs?.club_name?.toLowerCase().includes(debouncedSearch.toLowerCase())
      );
    }
    ...
    return filtered;
  }, [discussions, debouncedSearch, filterStatus, sortBy]);
      ...
      <div className="bg-white border rounded-lg p-6 mb-6">
        ...
        <div className="mt-4 flex items-center justify-between">
          <div className="text-sm text-gray-600">
            Showing {paginatedDiscussions.length} of {filteredDiscussions.length} filtered discussions
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => {
                ...
                setCurrentPage(1);
                if (typeof window !== "undefined") {
                  localStorage.removeItem("discussions_filterStatus");
                  localStorage.removeItem("discussions_sortBy");
                  localStorage.removeItem("discussions_itemsPerPage");
                }
              }}
